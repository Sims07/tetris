...
public record Board(
    ...
    public Board {
        ...
        Asserts.withContext(getClass())
            ...
            .isEqualTo(nbSlots, slots.size(), "Number of slots should be equal to %s and it is %s".formatted(nbSlots, slots.size()));
    }
    ...
    public static Map<Position, Optional<Tetromino>> emptySlots() {
        ...
        IntStream.range(0, NB_LINES).forEach(i -> {
            IntStream.range(0, NB_COLUMNS).forEach(j -> {
                emptyBoard.put(new Position(i, j), Optional.empty());
            });
        });
        return emptyBoard;
    }

    public Board fallTetromino(Tetromino tetromino) {
        final Tetromino movedTetromino = fallingTetromino()
            .map(Tetromino::moveDown)
            .orElse(tetromino);
        if (nextTetrominoPositionAvailable(movedTetromino)) {
            return new Board(
                boardId,
                moveTetrominoFromTo(tetromino, movedTetromino),
                Optional.of(movedTetromino)
            );
        } else {
            throw new TetrominoFixedException(tetromino, new Board(
                boardId,
                slots(),
                Optional.empty()
    ...
    private boolean nextTetrominoPositionAvailable(Tetromino movedTetromino) {
        return movedTetromino.positions()
            .stream()
            .map(position -> {
                if (position.x() < Board.NB_LINES) {
                    final boolean emptySlot = emptySlot(position);
                    final boolean tetrominoAlreadyAtThisSlot = tetrominoAlreadyAtThisSlot(movedTetromino, position);
                    LOGGER.debug("{},{},{}", position, emptySlot, tetrominoAlreadyAtThisSlot);
                    return emptySlot
                        ||
                        tetrominoAlreadyAtThisSlot;
                } else {
                    return false;
                }
            })
    ...
    private Map<Position, Optional<Tetromino>> moveTetrominoFromTo(Tetromino tetromino, Tetromino movedTetromino) {
