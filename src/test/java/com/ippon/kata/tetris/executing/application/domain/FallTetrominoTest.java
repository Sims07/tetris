...
class FallTetrominoTest {
  ...
  @Mock Boards boards;
  @Mock EventPublisher<TetrominoMovedEvent> eventPublisher;
  ...
  void givenNewFallingTetrominoOnEmptyBoard_fall_shouldSendMovedEvent() {
    ...
    final Board boardGot = new Board(boardId);
    given(boards.get(boardId)).willReturn(boardGot);
    given(boards.save(any())).willAnswer(args -> args.getArguments()[0]);
    final Shape shape = new Shape(ShapeType.S);
    final TetrominoMovedEvent tetrominoMovedEvent =
    ...
    final TetrominoMovedEvent movedEvent =
        fallTetromino.fall(
            boardId,
            new Tetromino(
                new TetrominoId(UUID.randomUUID()),
  ...
  void givenTetrominoFixedException_fall_shouldEmitMovedEventFixed() {
    ...
    final FallTetromino fallTetromino = new FallTetromino(eventPublisher, boards);
    final BoardId boardId = new BoardId(new GameId(UUID.randomUUID()));
    final Board boardGot =
        new Board(
            boardId,
            PositionSlot.emptySlots(),
            Optional.of(
                new Tetromino(
                    new TetrominoId(UUID.randomUUID()),
                    shape,
